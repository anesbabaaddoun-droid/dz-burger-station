...
export function OrderNotifier() {
  ...
  const knownIdsRef = useRef<Set<string> | null>(null); // null = not initialized yet
  const toastTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const { data: settings } = useFirestoreDoc('settings', 'config');
  const soundEnabled = settings?.soundNotifications !== false; // default true if not loaded yet
  ...
  const playBeep = () => {
    try {
      ...
      gain.gain.setValueAtTime(0.35, ctx.currentTime); // louder than before (was 0.15)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (knownIdsRef.current === null) {
        // First load: these are orders that arrived while we were away.
      ...
      // Subsequent updates: detect genuinely new order IDs.
